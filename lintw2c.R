...
plotdeg0 <- function(i, df)
{
    print(colnames(df[,c(i,3:5)])[1])
    gg <- ggplot(df[,c(i,3:5)], aes_string(x="Condition", y=colnames(df[,c(i,3:5)])[1], group="Pair"), colour="Condition") +
          ...
          theme_bw() +
          ggtitle(paste0(colnames(df[,c(i,3:5)])[1], " expression: ", hns, " samples, 2 conditions")) +
...
lp <- lapply(1:2, FUN=plotdeg0, df=df)
