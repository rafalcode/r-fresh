...
CairoPNG("covp0.png", 800, 800)
cp <- covplot(peak, weightCol="V5")
show(cp)
dev.off()

## ----fig.height=4, fig.width=10-----------------------------------------------
CairoPNG("covp1.png", 800, 800)
cp <- covplot(peak, weightCol="V5", chrs=c("chr17", "chr18"), xlim=c(4.5e7, 5e7))
show(cp)
dev.off()
...
CairoPNG("taghm.png", 800, 800)
tg <- tagHeatmap(tagMatrix)
show(tg)
dev.off()
...
CairoPNG("phm.png", 800, 800)
ph <- peakHeatmap(peak = files[[4]],
            TxDb = txdb,
            upstream = rel(0.2),
            downstream = rel(0.2),
            by = "gene",
...
CairoPNG("phm.png", 800, 800)
ph <- peakHeatmap_multiple_Sets(peak = files[[4]],
...
CairoPNG("pap.png", 800, 800)
# pp <- plotAvgProf(tagMatrix, xlim=c(-3000, 3000),
#             xlab="Genomic Region (5'->3')", ylab = "Read Count Frequency")
pp <- plotAvgProf(tagMatrixList, xlim=c(-3000, 3000))
show(pp)
dev.off()
stop("o")
