...
dds2 <- makeExampleDESeqDataSet(
  ...
  interceptSD = iSD,
  dispMeanRel = ourfunc,
  sizeFactors = sF # in the docs it put rep(1, m) this is an error, m is an option not a variable.
)
# above is all default and an equivalent is:
...
# how to examine. str() is pretty verbose
# just type dds, you get an overall summary
# mcols(dds) gives you an lm() type output
# colData() gives you the two conditions: first 6 are A, second six are B
# where to get the counts? KISS: counts()!

# but as it stands,
# there's been no calculation or fitting here.
# of course, because we've been assigning the results, typical for a simulation.
# so next run DESeq(dds) on them and reassign it to dds, as is their want.
# actuall estimateSizeFactors() seems to be the first stage of DESeq() and that can be used too.

# Mike love "this function is really quite basic .. use polyester instead!"
# https://support.bioconductor.org/p/77733/
# Nope, basic is fine.
