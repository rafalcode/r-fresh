...
# there is also a style to this code
library(Cairo)
...
for(i in 1:n.col){
  ...
  color.thisrange <- color.count + 1:n.row
  color.mod <- length(colors()) - color.count
  y.val <- ifelse(color.mod < n.row, n.row - color.mod + 1, 1)
  color.names <- as(color.df[color.thisrange, "name"], "character")
  labs.color.names <- paste0(color.names, "(", color.thisrange, ")")
  ...
  text(i-0.5, n.row:y.val, labels = labs.color.names, cex = 0.75, col = text.color)
}
